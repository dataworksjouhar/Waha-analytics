"""Shared helpers for silver transforms. Every transform reads its bronze
raw table fully into pandas, cleans it, and replaces the silver table
wholesale: bronze is the immutable source of truth, so rebuilding silver
from it each run is trivially idempotent, at the cost of redoing work that
an incremental design would skip.
...
SELECT or COPY over the largest table (pos_sales_raw, ~310k rows) can
exceed that window and gets cancelled outright, retries or not. Chunking
keeps every individual statement well under the limit regardless of total
table size.
"""
...
CONFIG_PATH = Path(__file__).resolve().parent.parent.parent / "config" / "client_waha.yml"
...
def read_bronze(engine: sqlalchemy.engine.Engine, table: str) -> pd.DataFrame:
    """Reads a bronze table via a server-side cursor so Postgres issues a
    DECLARE CURSOR plus repeated FETCH statements instead of one blocking
    SELECT. Each FETCH gets its own shot at the statement_timeout clock,
    so a table with more rows than fit in one 2-minute window still reads
    reliably; a plain SELECT * on pos_sales_raw does not."""
...
def replace_silver_table(engine: sqlalchemy.engine.Engine, table: str, df: pd.DataFrame) -> int:
    """Truncates the silver table and COPYs df in its place, in one
    ...
    def _write():
        with engine.begin() as conn:
            conn.execute(sqlalchemy.text(f"TRUNCATE {table}"))
            if len(df):
                out = df.copy()
